...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Simple CNN on CIFAR-10")
    ...
    parser.add_argument("--run-name", type=str, default="cnn")

    return parser.parse_args()


def plot_curves(history, save_dir: Path, run_name: str):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / f"{run_name}_loss_curve.png", dpi=200)
    plt.close()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title(f"{run_name} Accuracy Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_dir / f"{run_name}_accuracy_curve.png", dpi=200)
    plt.close()
...
def main():
    ...
    model = build_simple_cnn(num_classes=len(idx_to_class)).to(device)
    ...
    checkpoint_path = ckpt_dir / f"{args.run_name}_best.pth"
    history_path = log_dir / f"{args.run_name}_history.csv"
    ...
    for epoch in range(1, epochs + 1):
        ...
        plot_curves(history, fig_dir, args.run_name)

    print("========== Training Finished ==========")
    print(f"Best test accuracy: {best_acc:.4f}")
    print(f"Log saved to: {history_path.resolve()}")
